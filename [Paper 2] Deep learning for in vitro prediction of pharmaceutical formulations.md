...
### **6) 공식 예측의 딥러닝**  
제형 예측의 주요 어려움 중 하나는 신뢰할 수 있는 표준 제형 데이터가 부족하다는 것이다. 원인은 긴 실험주기와 높은 제형 개발 비용, 소수의 모델 약물에 초점을 맞춘 실험이다. 작은 데이터는 쉽게 과적합되고, 일반화되지 않기 때문에 예측은 더 작은 데이터 세트에서 더 약하다. 하여 이 문제를 해결하기 위해 1)훈련된 모델의 적용 가능성 영역을 추정하기 위해 유클리드 거리 사용, 2)최대 유사성 알고리즘 화학 데이터베이스에서 대표적인 화합물의 시험데이터를 선택해보았다. 그러나 두 방법 모두 매우 복잡한 제형 데이터가 아니라 약물 분자에 적합했다.
