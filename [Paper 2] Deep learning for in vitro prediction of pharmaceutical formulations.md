...
### **5) 딥러닝과 기존 머신러닝 방법의 비교**  
연구에서는 MLR, PLSR, SVM, ANN, RF, KNN 및 딥러닝 모델이 개발되었다. 여기에서 3개의 데이터세트(trian/validation/test)는 개인적인 전문지식없이 MD-FIS알고리즘을 두번 사용하여 분할되었다. SRMT예측에서는 각 기계학습 방법을 사용하여 4개의 시점(2,4,6,8시간)에 대해 4개의 모델을 구축하였다.  

[OFDF 방식 데이터에 대한 기존 머신러닝모델 및 심층 신경망의 결과]
![e](https://user-images.githubusercontent.com/59431387/104416807-c193d800-55b7-11eb-8d7a-da2b8e0a7f57.PNG)  
OFDF예측에서 선형/비선형의 기존 머신러닝 방법을 기반으로 하는 모든 모델의 정확도는 검증/테스트 세트에서 약 70%에 불과하였다. 특히, MLR모델은 OFDF 유효성 검사 및 테스트 세트에서 다른 기존 기계 학습 모델보다 상대적으로 낮았다.  

[SRMT 방식 데이터에 대한 기존 머신러닝모델 및 심층 신경망의 결과]
![f](https://user-images.githubusercontent.com/59431387/104416942-f7d15780-55b7-11eb-8e8b-a1237f02db25.PNG)  
SRMT예측에서 기존 기계학습 모델은 SRMT검증 및 테스트 세트에서 25%에서 55%범위의 정확도로 예측하였다.  
**결론적으로 6가지의 기존 기계학습 방법 모두 OFDF 및 SRMT 공식 예측에 충분한 성능을 달성 할 수 없었다.**  

DNN모델은 이전 기계학습 모델 훈련에 사용된 데이터세트를 동일하게 사용하였다. 딥러닝 기법을 사용하여 4개의 시점(2,4,6,8시간)에 대해 다중 레이블 모델을 구축하였다. 위의 두 표와 같이 심층 신경망의 모든 예측 정확도는 80%이상으로 공식 예측의 요구사항을 충족하였다. OFDF 및 SRMT 예측 모두에서 딥러닝은 train/validation/test데이터에서 가장 높은 정확도를 얻었다. 이는 딥러닝이 인공적인 특성 엔지니어링 없이 낮은 수준의 표현을 높은 수준의 특성으로 변환할 수 있기 때문에 다른 기존 기계학습 방법을 능가할 수 있었다. 하여 SRMT예측에서 다른 기계 학습방법보다 딥러닝의 성능이 크게 향상되었다.  
**결론적으로 딥러닝이 여러 작업간에 공유된 정보를 활용할 수 있기 때문에 다중레이블 공식 예측에서 모델 정확도를 크게 향상시킬 수 있었다.** 아래의 두 표를 통해 딥러닝의 예측성능이 만족스러움을 객관전으로 알 수 있다.  

[OFDF테스트 세트에서 제형의 실험 및 딥러닝 예측 분해 시간]
![g](https://user-images.githubusercontent.com/59431387/104418167-d5d8d480-55b9-11eb-8e10-6bc155f40e2d.PNG)     


[SRMT테스트 세트에서 제형의 실험 및 딥러닝 예측 누적 약물 방출 누적 커브 예측]
![h](https://user-images.githubusercontent.com/59431387/104418327-159fbc00-55ba-11eb-89c9-e642c059ad3f.PNG)     


[OFDF 및 SRMT의 train/validation/test 세트에 대한 실험 결과 와 딥러닝 예측 결과 간의 관계]
![i](https://user-images.githubusercontent.com/59431387/104418727-bc845800-55ba-11eb-80d8-a7cb2974ed6b.PNG)     
ㄴOFDF train/validation/test 세트에서 분해시간 실험 및 딥러닝 예측값 간의 관계 (점선은 실험값±10초를 의미)  


![j](https://user-images.githubusercontent.com/59431387/104418730-bdb58500-55ba-11eb-87f0-df74d45a5ded.PNG)     
ㄴSRMT의 train세트에서 4개의 시점(2,4,6,8시간)누적 약물 방출 백분율의 실험 및 딥러닝 예측 값 간의 관계  (A:2, B:4, C:6, D:8시간을 의미)  


![k](https://user-images.githubusercontent.com/59431387/104418731-bdb58500-55ba-11eb-8aaf-d97f76b28392.PNG)  
ㄴSRMT의 validation세트에서 4개의 시점(2,4,6,8시간)누적 약물 방출 백분율의 실험 및 딥러닝 예측 값 간의 관계  (A:2, B:4, C:6, D:8시간을 의미)  


![l](https://user-images.githubusercontent.com/59431387/104418732-be4e1b80-55ba-11eb-9e00-6ed28d75495a.PNG)  
ㄴSRMT의 test세트에서 4개의 시점(2,4,6,8시간)누적 약물 방출 백분율의 실험 및 딥러닝 예측 값 간의 관계  (A:2, B:4, C:6, D:8시간을 의미)  


출력을 예측할 수 있는 입력 특성의 선형 조합을 학습하기 위해 다중 선형 회귀 접근법이 사용되었다. 다중 선형 회귀의 가중치와 편향은 최소제곱법을 사용하여 직접 계산할 수 있다. 특히 가중치는 예측에서 입력특성의 중요성을 나타낼 수 있기 때문에 다른 비선형 기계학습모델보다 더 나은 해석성을 갖는다. 그러나 다중 선형 회귀 및 부분 최소 제곱 회귀는 선형 함수에만 적합하고, 주요 특성 간의 관계는 복잡하고, 비선형적이다.  
랜덤 포레스트는 앙상블의 한 종류로 여러 기본 학습자를 결합하여 다른 기본 학습자보다 모델의 더 나은 일반화 능력을 얻을 수 있다. 많은 학습 작업에서 다른 앙상블 학습 모델보다 더 나은 성능을 보여준다. 또한 랜덤포레스트에서 기본 학습자의 다양성은표본 교란 뿐만 아니라 속성교란에서도 발생하므로 기본 학습자 간의 차이가 증가하고 일반화 능력이 더욱 향상된다.  
서포트 벡터 머신은 원본 공간의 샘플을 더 높은 차원의 특징 공간으로 매핑하므로 샘플을 더 높은 차원의 특징 공간으로 나눌 수 있다. 그러나 기존의 머신러닝은 주관적이 전문가 경험에 의해 설계된 기능 추출기에 크게 의존한다.  

인공신경망은 은닉층과 은닉 노드의 증가에 따라 능력이 향상된다. 모델 용량이 클수록 모델이 더 복잡한 기능을 수행할 수 있다. 따라서 더 많은 히든 레이어를 포함하는 딥러닝은 다중 추상화 및 특징 추출을 수행하여 딥러닝이 얕은 인공신경망보다 더 높은 정확도로 더 복잡한 작업을 수행할 수 있도록 해야한다.  

### **6) 공식 예측의 딥러닝**  
제형 예측의 주요 어려움 중 하나는 신뢰할 수 있는 표준 제형 데이터가 부족하다는 것이다. 원인은 긴 실험주기와 높은 제형 개발 비용, 소수의 모델 약물에 초점을 맞춘 실험이다. 작은 데이터는 쉽게 과적합되고, 일반화되지 않기 때문에 예측은 더 작은 데이터 세트에서 더 약하다. 하여 이 문제를 해결하기 위해 1)훈련된 모델의 적용 가능성 영역을 추정하기 위해 유클리드 거리 사용, 2)최대 유사성 알고리즘 화학 데이터베이스에서 대표적인 화합물의 시험데이터를 선택해보았다. 그러나 두 방법 모두 매우 복잡한 제형 데이터가 아니라 약물 분자에 적합했다. 딥러닝은 입력공간이 불균형한 소량의 데이터에서 정확하게 예측하기 어렵고, 회귀 문제에 대한 두가지 공통평가 메트릭은 제약 예측 모델의 성능을 반영할 수 없었다. 그렇기 때문에 모델 성능을 평가하려면 특정 기준을 도입하여야 한다.  

현재 공식 예측의 딥러닝 접근법은 2가지가 보고되어있다.

    1. 기능 서낵 모델은 입력변수를 최소화 하기 위해 개발
    2. 7가지의 기계학습 접근 방식(입체파, RF, ANN, 다변량 적응 회귀 스플라인, 분류 및 회귀 트리, 퍼지논리 및 진화 계산의 하이브리드 시스템)을 비교
    (입력 벡터에는 특정 시점에서 320개의 기능과 745개의 릴리스 데이터가 포함됨)

최종 결과, RF가 0.692의 결정계수로 최상의 성능을 갖는 것으로 나타났다. 모델의 예측은 2/3공식에만 적합했고, 나머지 1/3에는 높은 오차를 가졌다. 총 14개의 단백질 중 4개의 단백질 용해프로파일은 최종 모델에서 권장되지 않았고, 제형 개발에는 고차원적 요인이 포함되어 있어 복잡한 데이터가 생성되었다. 딥러닝은 특징 선택 모델을 명시적으로 제공하지 않고도 데이터에서 높은 수준의 특징을 자동으로 학습할 수 있기 때문에 특징 추출기와 딥러닝 모델을 결합하는 것은 불필요하고 부적절하다는 결론이다. 따라서, 이 연구는 제형 예측을 위한 방법론에 대한 개선이 여전히 필요하다.  

최근 MD-FIS를 이용한 딥러닝이 구강 속 붕정제제예측에 적용되었다. 딥러닝은 특징 추출기를 설계하지 않고도 자동으로 특징을 추출할 수 잇기 때문에 딥러닝 예측이 ANN보다 더 나은 것으로 나타났다.

<br/></br>
## **4. 결론**  
작은 데이터에 대한 제약 제제를 예측하기 위한 딥러닝 모델이 성공적으로 개발되었다. 모델의 우수한 일반호화 성능은 외부 데이터 세트를 통해 입증되었다. 제안된 모델은 다른 기계 학습 방법으로 훈련 된 모델보다 회귀 문제의 주요 특성을 더 효과적으로 예측할 수 있다.  
현대의 성공적인 의약품 개발은 의약품 개발 프로세스 전반에 걸쳐 QbD개념을 통합해야한다. 기계학습방법은 생체 내 및 test세트 예측 뿐만 아니라 조제 및 공정데이터를 기반으로 하는 특성, 제약 실험 설계를 지원하고 전체 제품 주기에서 제품 품질을 제어하는데 도움이 된다. 딥러닝은 QbD구현에 큰 잠재력을 보여주기 때문에 딥러닝을 통해 의약품 개발 일정을 크게 단축하고 재료 사용량을 줄일 것을 기대한다. 또한 약학과 인공지능의 학제 간 통합은 제약연구의 패러다임을 경험의존 연구에서 데이터 기반 방법론으로 바꿀 수 있을 것이다.
