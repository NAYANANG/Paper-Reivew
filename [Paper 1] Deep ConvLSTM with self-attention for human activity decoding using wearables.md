...
정확성 면에서 새로운 접근 방식은 기존 문헌에서 발견되는 모든 방법보다 우수하다. 제안된 방법에 의해 얻어진 개선은 WISDM을 제외한 모든 데이터 세트에서 통계적으로 유의하다. 모든 데이터셋에서 수작업으로 조작된 기능을 갖춘 MLP를 사용하여 가장 낮은 성능을 달성할 수 있다. 그러나 통계에서 특히 UTD-MHAD1에 대해 앙상블학습을 사용할 때 MLP보다 모든 데이터 세트의 정확도가 향상되는 것을 볼 수 있다. 앙상블 학습과 2D CNN사이에 큰 차이를 발견하지 못했으며 서로 다른 데이터 세트에 대한 혼합 결과를 확인할 수 있었다.

<br/></br>
## **5. 결론**  
이 논문에서는 여러 웨어러블 센서로부터 데이터의 시공간적 측면을 고려하고 인간활동을 해독하기 위한 조합을 식별하기 위해 자기주의 메커니즘을 사용하는 딥러닝 아키텍처를 제안했다. CNN과 RNN계층은 각각 공간적 및 시간적특징을 인코딩하지만, 셀프 attention계층은 CNN과 RNN에 의해 생성된 센서 시계열의 임베딩에서 기능 표현을 생성한다. 이 실험은 제안된 아키텍처가 MHEACE, USC HAD, U에서 서로 다른 샘플 생성체계에 대해 유사한 성능을 제공한다는 것을 보여준다.  
제안된 아키텍처의 성능을 CNN 및 RNN모델에 기초한 기준선과 비교하고, 이전의 접근법과 비교하여 제안된 접근법이 활동인식에 대한 정확성의 향상으로 이어진다는 것을 발견했다. 그러므로, **제안된 아키텍처는 인간활동 인식에 광범위한 응용 프로그램을 가지고 있으며 많은 수의 센서와 시점을 통해 확장될 수 있다.**
