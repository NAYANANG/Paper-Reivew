...
## **7. CDC의 적용**  
1. 네트워크 기능 가상화 (NFV)  
    NFV는 기존에 독점 하드웨어에서 실행되었던 네트워크 서비스를 가상화한 것이다. 
        
    <NFV의 특징>
    
        - 가상화기술을 활용하므로 네트워킹 서비스의 관리 및 운영을 단순화 함  
        - 네트워크 기능(NF)이 작동하는 물리적 네트워크 장비에서 분리됨  
        - NF는 Commercial Off The Shelf(COTS) 물리적 소스에서 지원하는 CNF에 매핑됨  
        - 하드웨어 구성요소와 분리되므로 VNF는 필요한 하드웨어 리소스가 있는 분산 네트워크 위치에서 쉽게 구현할 수 있음  
        - VNF배치의 유연성으로 인해 NFV는 운영 및 자본 비용을 현저히 감소시킴  
        - COTS리소스를 쉽게 인스턴스화하여 필요한 네트워크 연결 서비스를 제공할 수 있으므로 새로운 네트워킹 서비스의 개발이 더 빠름  
        
    - NFV의 성능을 제한하는 요인 : COTS하드웨어 리소스의 신뢰성 (하드웨어 장애는 NFV구현에 영향을 미치고 서비스 제공을 방해함)
    - 성능 제한 요인 해결 방안 : NFV의 핵심 응용 분야인 CRAN (Cloud Radio Access NEtwork)아키텍처의 복잡한 업링크 채널 디코딩을 고려
        
                       - 사용자는 RRH를 통해 클라우드와 통신  
                       - RRH에 의해 수신된 데이터프레임은 서로 다른 VM에 배포되기 전에 대수구조를 활용하여 인코딩됨  
       시뮬레이션 결과, 다양성 기반 접근 방식보다 클라우드에서 디코딩 오류가 덜 발생함.  
                       
    
2. 엣지 컴퓨팅  
    최종 장치의 향상된 감지기능으로 오늘날 네트워크 에지에서 엄청난 양의 데이터가 생성된다. 따라서, 클라우드로의 흐름을 계산하는 전통적인 방식은 지속이 불가능하다. 따라서 클라우드의 흐름을 계산하는 전통적인 방식은 지속이 불가능하다. 이를 위해서 데이터가 생성되는 네트워크의 에지에 더 가깝게 계산이 수행되는 에지컴퓨팅으로의 패러다임 전환이 필요하다.  
    1. 이 연구에서는 코딩된 무선 분산 컴퓨팅(CWDC) 프레임워크를 제안한다. 시스템모델은 여러 장치로 구성된다. 추론모델은 각 작업자에 대해 분할 및 저장될 수 있는 반면, 결과의 협력 계산은 MapReduce프레임워크에 따라 구현될 수 있다. 제안된 프레임워크는 네트워크 크기 및 작업자의 스토리지 크기와 무관한 통신부하를 달성한다. 또한 CWDC프레임워크는 일반화되어 다양한 유형의 애플리케이션에 적용될 수 있다.  
    2. 이 연구는 선형 회귀 모델을 훈련하기 위해 네트워크의 작업자에 걸친 경사하강 계산의 분포를 고려한다. 제안된 이기종 코드 경사 하강법(HCGD)은 지수분포가 이동한 장치의 계산지연을 모델링하여 각 작업자에게 최적의 부하 분할을 할당한다. 데이터 프라이버시를 고려하여 프라이버시를 보존하는 선형회귀를 위한 CFL(Coded Federated Learning)접근법을 제안한다. Federated Learning(FL)은 제안한 개인보호 분산 머신러닝 패러다임으로 ㄷ이터 소유자의 민감한 데이터는 로컬에 보관되고 모델 매개 변수만 집계하기 위해 중앙서버로 전송되어 모델을 업데이트한다. 하지만 FL은 여전히 장치를 망치거나 통신 비효율성 문제가 있다. 따라서 제안된 CFL방식에서 각 데이터 소유자는 먼저 중앙서버로 전송하기 위해 로컬데이터에서 패리티 데이터를 생성한다. 중앙서버에서 기울기는 페리티 데이터에서 동시에 계산되므로 모델 업데이트 완료를 위해 데이터 소유자의 기울기 하위 집합만 수신하면 된다. 다음은 CFL체계의 구현이다.  
    ![n](https://user-images.githubusercontent.com/59431387/110245219-99b46580-7fa5-11eb-97ae-806c9f830362.PNG)  
    CFL접근방식은 코딩되지 않은 접근방식보다 거의 4배빠르게 수렴하는 것으로 나타났다. 하지만 페리티 데이터 생성 및 전송과 관련된 통신/계산비용은 이 연구에서 정교화되지 않았다.
